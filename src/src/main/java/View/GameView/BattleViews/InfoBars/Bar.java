package View.GameView.BattleViews.InfoBars;
...
public class Bar extends VBox {
    ...
    public void updateHPBars(int targetHP, Runnable onFinish) {
        int maxHP = pokemon.getMaxHP();
        int currentDisplayedHP = (int) Math.round(HPBar.getProgress() * maxHP);
        targetHP = Math.max(0, Math.min(maxHP, targetHP));
        double startProgress = (double) currentDisplayedHP / maxHP;
        double endProgress = (double) targetHP / maxHP;

        double step = 0.02;
        int steps = (int) Math.ceil(Math.abs(endProgress - startProgress) / step);
        if (steps <= 0) {
            health.setText(targetHP + "/" + maxHP);
            ApplyColor(new AtomicInteger(targetHP), maxHP, HPBar);
            if (onFinish != null) onFinish.run();
            return;
        }

        AtomicInteger displayedHP = new AtomicInteger(currentDisplayedHP);
        Timeline timeline = new Timeline();
        KeyFrame keyFrame = new KeyFrame(Duration.millis(50), e -> {
            double progress = HPBar.getProgress();
            double newProgress = progress < endProgress ? Math.min(progress + step, endProgress) : Math.max(progress - step, endProgress);
            HPBar.setProgress(newProgress);
            int hp = (int) Math.round(newProgress * maxHP);
            displayedHP.set(hp);
            health.setText(hp + "/" + maxHP);
            ApplyColor(displayedHP, maxHP, HPBar);
        });
...
//                getTextBubble().showMessage(pName + " levels up !");
...
//                    statBubble.showMessage(statMessage(pokemon));
    ...
    public void refreshStatus() {
        ...
        if (st == Status.freeze || st == Status.burned || st == Status.paralyzed || st == Status.poisoned || st == Status.badlyPoisoned || st == Status.asleep) {
            statusImg.setImage(new Image(Objects.requireNonNull(getClass().getResourceAsStream("/images/" + st + ".png"))));
        } else {
            statusImg.setImage(null);
            hBox1.setSpacing(30);
        }
